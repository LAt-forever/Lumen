...
from service.models import AgentProfile, RerankerProfile
...
def list_agent_profiles(db: Session = Depends(get_db)):
    return [_agent_profile_read(profile, AgentRepository(db)) for profile in AgentRepository(db).list_profiles()]


@router.post("/profiles", response_model=AgentProfileRead)
def create_agent_profile(data: AgentProfileCreate, db: Session = Depends(get_db)):
...
def update_agent_profile(profile_id: int, data: AgentProfileUpdate, db: Session = Depends(get_db)):
    repo = AgentRepository(db)
    try:
...
def activate_agent_profile(profile_id: int, db: Session = Depends(get_db)):
    repo = AgentRepository(db)
    try:
...
def run_agent(data: AgentRunRequest, db: Session = Depends(get_db)):
...
def list_tool_logs(limit: int = 50, db: Session = Depends(get_db)):
    bounded_limit = min(max(limit, 1), 100)
    return AgentRepository(db).list_tool_logs(limit=bounded_limit)


@router.get("/reranker-profiles", response_model=list[RerankerProfileRead])
def list_reranker_profiles(db: Session = Depends(get_db)):
    return [_reranker_profile_read(profile) for profile in AgentRepository(db).list_reranker_profiles()]


@router.post("/reranker-profiles", response_model=RerankerProfileRead)
def create_reranker_profile(data: RerankerProfileCreate, db: Session = Depends(get_db)):
    return _reranker_profile_read(AgentRepository(db).create_reranker_profile(data))


@router.patch("/reranker-profiles/{profile_id}", response_model=RerankerProfileRead)
def update_reranker_profile(profile_id: int, data: RerankerProfileUpdate, db: Session = Depends(get_db)):
    repo = AgentRepository(db)
    try:
...
def activate_reranker_profile(profile_id: int, db: Session = Depends(get_db)):
    repo = AgentRepository(db)
    try:
