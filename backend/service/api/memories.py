...
def list_candidates(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return MemoryRepository(db, user_id=current_user.id).pending_candidates()


@router.post("/candidates/{candidate_id}/confirm", response_model=MemoryRead)
def confirm_candidate(candidate_id: int, data: MemoryUpdate | None = None, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    service = MemoryService(MemoryRepository(db, user_id=current_user.id))
    try:
...
def ignore_candidate(candidate_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    service = MemoryService(MemoryRepository(db, user_id=current_user.id))
    try:
...
def list_memories(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return MemoryRepository(db, user_id=current_user.id).active_memories()


@router.get("/duplicate-suggestions", response_model=list[MemoryDuplicateSuggestionRead])
def duplicate_suggestions(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return MemoryService(MemoryRepository(db, user_id=current_user.id)).duplicate_suggestions()


@router.get("/graph/hubs", response_model=MemoryGraphRead)
def memory_hub_graph(limit: int = 5, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    service = MemoryService(MemoryRepository(db, user_id=current_user.id))
    return service.build_hub_graph(limit=limit)


@router.patch("/{memory_id}", response_model=MemoryRead)
def update_memory(memory_id: int, data: MemoryUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    service = MemoryService(MemoryRepository(db, user_id=current_user.id))
    try:
...
def forget_memory(memory_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    service = MemoryService(MemoryRepository(db, user_id=current_user.id))
    try:
...
def merge_memory(memory_id: int, data: MemoryMerge, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    service = MemoryService(MemoryRepository(db, user_id=current_user.id))
    try:
...
def list_memory_relations(memory_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    repository = MemoryRepository(db, user_id=current_user.id)
    service = MemoryService(repository)
    if not repository.exists_active(memory_id):
...
def create_memory_relation(memory_id: int, data: MemoryRelationCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    service = MemoryService(MemoryRepository(db, user_id=current_user.id))
    try:
...
def forget_memory_relation(memory_id: int, relation_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    service = MemoryService(MemoryRepository(db, user_id=current_user.id))
...
def memory_graph(memory_id: int, depth: int = 2, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    service = MemoryService(MemoryRepository(db, user_id=current_user.id))
    try:
...
def promote_duplicate_to_relation(source_memory_id: int, target_memory_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    service = MemoryService(MemoryRepository(db, user_id=current_user.id))
    try:
